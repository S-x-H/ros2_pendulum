...
Plotter::Plotter() : Node("plotter") {
  ...
  angularPosSubFilt_.subscribe(this, "pendulum_angular_position",
                               qos.get_rmw_qos_profile());
  angularVelSubFilt_.subscribe(this, "pendulum_angular_velocity",
                               qos.get_rmw_qos_profile());
  ...
  if (useConstTorque) {
    ...
    poseVelFilter_.reset(new PoseVelSync(PoseVelPolicy(10), angularPosSubFilt_,
                                         angularVelSubFilt_));
    poseVelFilter_->registerCallback(std::bind(&Plotter::poseVelCb, this,
                                               std::placeholders::_1,
                                               std::placeholders::_2));
  } else {
    ...
    poseVelTorqueFilter_.reset(
        new PoseVelTorqueSync(PoseVelTorquePolicy(10), torqueSubFilt_,
                              angularPosSubFilt_, angularVelSubFilt_));
    poseVelTorqueFilter_->registerCallback(
        std::bind(&Plotter::poseVelTorqueCb, this, std::placeholders::_1,
  ...
  try {
    ...
    if (fileStream_.is_open()) {
      ...
      RCLCPP_ERROR(this->get_logger(), "Unable to create file: %s",
                   logName.c_str());
    }
  } catch (const std::exception &e) {
    RCLCPP_ERROR(this->get_logger(),
                 "Exception thrown creating file: %s. Exception was: %s",
                 logName.c_str(), e.what());
  }
...
void Plotter::poseVelCb(
  ...
  fileStream_ << std::setfill('0') << std::setw(9)
              << posMsg->header.stamp.nanosec;
