...
Clock::Clock() : Node("clock") {
  ...
  timer_ = this->create_wall_timer(std::chrono::milliseconds(updateStep),
                                   std::bind(&Clock::timerCallback, this));

  rclcpp::QoS qos(rclcpp::KeepLast(10));
  clockPub_ = this->create_publisher<sensor_msgs::msg::TimeReference>(
